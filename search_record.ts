...
common.methodCall('authenticate', [process.env.ODOO_DB, process.env.ODOO_USERNAME, process.env.ODOO_PASSWORD, {}], (err, uid) => {
  client.methodCall('execute_kw', [process.env.ODOO_DB, uid, process.env.ODOO_PASSWORD, 'approval.request', 'search_read', [[]], { limit: 10, order: 'id desc', fields: ['id', 'name', 'x_studio_cuenta_catastral_1'] }], (err, records) => {
